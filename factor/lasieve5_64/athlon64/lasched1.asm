...
ifelse(nt_sched,1,
	`function_head(lasched`'ot`'_1nt)',
	`function_head(lasched`'ot`'_1)')
	pushq %rbx
	shlq $n_i_bits,ij_ub
	pushq %r12
	pushq %r13
	...
	movl (ri),ad
	movl 4(ri),bd
	jbe lasched`'ot`'_1_fbi_loop_end
lasched`'ot`'_1_fbi_loop:
	movl ad,aux0d
	movl bd,aux1d
	andl $ot_mask,aux0d
	andl $ot_mask,aux1d
...
dnl smjs	prefetchw 64(ij_ptr)
	...
	movl (ri),ad
	movl 4(ri),bd
	ja lasched`'ot`'_1_fbi_loop
lasched`'ot`'_1_fbi_loop_end:
	movq ri,%rax
	popq %rbp
	popq %rbp
	popq %r15
...
ifelse(ot,2,`
lasched2_1_exception:
	movl (ri),aux0d
	cmpl aux0d,4(ri)
	jb lasched2_1_afterexception
	cmpl ad,bd
	movl $n_i,ijd
	movl $0,aux1d
	cmovel 4(ri),ijd
