...
static u32_t b;
static mpz_t first_spq,first_spq1,last_spq,sieve_count,first_root;
static u32_t spq_count;

static mpz_t m,N,aux1,aux2,aux3,aux4,sr_a,sr_b;
/* The polynomial. */
static mpz_t *(poly[2]);
...
@ All prime divisors of special q are stored in the following:

@c
typedef struct sFBstruct {
  u32_t p,r;
} *sFBptr;
static volatile sFBptr sFB[2];
static volatile u32_t sFBs[2];
...
/*
spq_q contains the n_spq_q special q, stored as difference q-first_spq1,
spq_p contains all n_spq_p prime divisors of these special q,
the divisors are between spq_pmin and spq_pmax,
spq_p_roots contains the roots of the polynomial on side special_q_side
for the primes in spq_p.
*/
/* divisors of the current special q */
static u64_t curspq_p[20];
/* n_curspq_p divisors, n_curspq_p32 of them below 2^32 */
static u32_t n_curspq_p32, n_curspq_p, curspq_p_ind[20];
/* product of first 21 odd primes > 2^100 */
static u32_t *spq_q;
static u32_t n_spq_q;
static u32_t spq_ndiv_max;
static u64_t *spq_p, spq_pmin, spq_pmax;
static u32_t n_spq_p, spq_q_ind;
static u64_t **spq_p_roots;
/*
spq_init generates the list of special q, their prime divisors and
modular roots.
spq_next switches to the next special q and puts the n_curspq_p divisors of
this q in curspq_p.
spq_roots computes the modular roots modulo the current special q.
*/
static void spq_init(mpz_t spq, mpz_t last);
static void spq_next(mpz_t spq);
static u32_t spq_roots(mpz_t **rptr, size_t *allocptr);
...
setbuf(stderr,NULL);
  @<Getopt@>@;
  siever_init();
  mpz_set(special_q,first_spq1);
  if (mpz_sgn(sieve_count)) spq_init(special_q,last_spq);
  @<Open the output file@>@;
  @<Generate factor bases@>@;
  @<Remove spq from factor bases@>@;
  @<Rearrange factor bases@>@;
  if(mpz_sgn(sieve_count)==0) exit(0);
  ...
  if(mpz_cmp(special_q,last_spq)>=0 && all_spq_done !=0) exit(0);
...
@<Do the lattice sieving between |first_spq| and |last_spq|@>=
{
  mpz_t *r; /* The prime ideals above this special q number. */
  size_t r_alloc=0;
  last_clock=clock();
  n_spq=0;
  n_spq_discard=0;
  ...
  for(;
      mpz_cmp(special_q,last_spq)<0 && mpz_sgn(special_q) != 0;
      spq_next(special_q),mpz_set_ui(first_root,0)) {
    u32_t nr;

    special_q_log=log(mpz_get_d(special_q));
    if(cmdline_first_sieve_side==USHRT_MAX) {
      double nn[2];
      u32_t s;

      for(s=0;s<2;s++) {
	nn[s]=log(poly_norm[s])+(special_q_side == s ? 1 : special_q_log);
    ...
    nr=spq_roots(&r,&r_alloc);

    for(root_no=0;root_no < nr; root_no++) {
      u32_t termination_condition;

      if(mpz_cmp(r[root_no],first_root)<0) continue;
	...
	if(termination_condition==USER_INTERRUPT)
	  ...
	  char *cmd,*qstr,*rstr;
          qstr=mpz_get_str(NULL,10,special_q);
          rstr=mpz_get_str(NULL,10,r[root_no]);
	  asprintf(&cmd,"touch badsched.%s.%u.%s.%s",basename,
		   special_q_side,qstr,rstr);
	  system(cmd);
	  free(cmd); free(qstr); free(rstr);
...
//gmp_printf("qr: %Zd %Zd\n",special_q,r[root_no]);
      if (reduce2gmp(&a0,&b0,&a1,&b1,special_q,r[root_no],(double)(sigma*sigma))) { 
        n_spq_discard++;
        continue;
      }
      n_spq++;
      @<Calculate |spq_i| and |spq_j|@>@;
      gmp_fprintf(ofile,"# Start %Zd %Zd (%lld,%lld) (%lld,%lld)\n",
	      special_q,r[root_no],a0,b0,a1,b1);
      @<Do the sieving and td@>@;
      gmp_fprintf(ofile,"# Done %Zd %Zd (%lld,%lld) (%lld,%lld)\n",
	      special_q,r[root_no],a0,b0,a1,b1);
...
/* seems to be ok for small intervals  */

/* SPQ_SIEVE_MAX must be at least 2^16 */
#define SPQ_SIEVE_MAX 16777216
@
@c
void spq_init(mpz_t spq, mpz_t last)
{
  u32_t p, x, x0, len, *rhelp, i, j, nr, plen0, pmax, ndiv;
  u64_t p64, x64, *rhelp64;
  char *qarray;
  size_t p_alloc=0, q_alloc=0;

  if (mpz_cmp(spq,first_spq1))
    Schlendrian("spq_init failed, wrong first_spq1\n");
  initprime32(&special_q_ps);
  mpz_sub(aux1,last,spq);
  mpz_fdiv_q_2exp(aux2,aux1,32);
  if (mpz_sgn(aux2)) complain("q-interval too long\n");
  len=(u32_t)mpz_get_ui(aux1);
  qarray=(char *)xmalloc(len*sizeof(char));
  memset(qarray,1,len*sizeof(char));
  pmax=SPQ_SIEVE_MAX;
  if (mpz_sizeinbase(last,2)<=64) {
    pmax=(u32_t)ceil(sqrt(mpz_get_d(last)+0.1));
    if (pmax>SPQ_SIEVE_MAX) pmax=SPQ_SIEVE_MAX;
  }

  for(p=firstprime32(&special_q_ps);((u64_t)p<spq_pmin) && (p<pmax);
      p=nextprime32(&special_q_ps)) {
    x=(u32_t)(mpz_fdiv_r_ui(aux1,spq,(ulong)p));
    if (x) x=p-x;
    while (x<len) { qarray[x]=0; x+=p; }
  }
  rhelp=xmalloc(poldeg_max*sizeof(*rhelp));
  rhelp64=xmalloc(poldeg_max*sizeof(*rhelp64));
  for(;p<pmax;p=nextprime32(&special_q_ps)) {
    x0=(u32_t)(mpz_fdiv_r_ui(aux1,spq,(ulong)p));
    if (x0) x0=p-x0;
    x=x0;
    if ((u64_t)p>=spq_pmax) {
      while (x<len) { qarray[x]=0; x+=p; }
    ...
    while (x<len) { if (qarray[x]) break; x+=p; }
    if (x<len) {
      adjust_bufsize((void**)&spq_p,&p_alloc,(size_t)(n_spq_p+1),
                     16,sizeof(u64_t));
      nr=root_finder(rhelp,poly[special_q_side],poldeg[special_q_side],p);
      if ((nr) && (rhelp[nr-1]==p)) nr--;
      if (!nr) {
        x=x0;
        while (x<len) { qarray[x]=0; x+=p; }
        continue;
      }
      spq_p[n_spq_p]=(u64_t)p;
      n_spq_p++;
    }
  }
  ...
  plen0=n_spq_p;
  n_spq_q=1;
  adjust_bufsize((void**)&spq_q,&q_alloc,(size_t)(n_spq_q),
                 16,sizeof(u32_t));
  spq_q[0]=0;
  for (i=0; i<len; i++)
    if (qarray[i]) {
      mpz_add_ui(aux1,spq,(ulong)i);
      ndiv=0;
      for (j=0; j<plen0; j++) {
        mpz_set_ull(aux3,spq_p[j]);
        mpz_fdiv_qr(aux2,aux3,aux1,aux3);
        if (mpz_sgn(aux3)==0) {
          mpz_set(aux1,aux2);
          mpz_set_ull(aux3,spq_p[j]);
          mpz_fdiv_r(aux2,aux1,aux3);
          if (mpz_sgn(aux2)==0) {
            mpz_set_ui(aux1,0);
            break;
          }
          ndiv++;
          if (mpz_cmp_ui(aux1,1)==0) break;
        }
      }
      if (!mpz_sgn(aux1)) continue;
      if (ndiv>spq_ndiv_max) continue;
      if (mpz_cmp_ui(aux1,1)) @<Try to factor special q@>@;

      adjust_bufsize((void**)&spq_q,&q_alloc,(size_t)(n_spq_q+1),
                     16,sizeof(u32_t));
      spq_q[n_spq_q]=i;
      n_spq_q++;
      if (n_spq_q>spq_count) break;
    }
  if (n_spq_q==1) {
    logbook(0,"No special q in this range\n");
    exit(0);
  }

/* remove unused divisors and duplicates from spq_p list */
  i=0;
  while (i<n_spq_p) {
    p64=spq_p[i];
    mpz_set_ull(aux3,p64);
    mpz_fdiv_r(aux2,spq,aux3);
    x64=mpz_get_ull(aux2);
    if (x64) x64=p64-x64;
    for (j=1; j<n_spq_q; j++) if (x64==(((u64_t)spq_q[j])%p64)) break;
    if (j>=n_spq_q) {
      n_spq_p--;
      if (i<n_spq_p) spq_p[i]=spq_p[n_spq_p];
    } else i++;
  }
  qsort(spq_p,(size_t)n_spq_p,sizeof(*spq_p),u64_cmp012);
  for (i=1,j=1; i<n_spq_p; i++)
    if (spq_p[i-1]<spq_p[i]) {
      if (j<i) spq_p[j]=spq_p[i];
      j++;
    }
  n_spq_p=j;

/* store len as final special_q */
  adjust_bufsize((void**)&spq_q,&q_alloc,(size_t)(n_spq_q+1),
                 16,sizeof(u32_t));
  spq_q[n_spq_q]=len;
  n_spq_q++;
  ...
  spq_p_roots=(u64_t **)xmalloc(n_spq_p*sizeof(u64_t *));
  for (i=0; i<n_spq_p; i++) {
    p64=spq_p[i];
    nr=root_finder64(rhelp64,poly[special_q_side],poldeg[special_q_side],p64);
    if (!nr) Schlendrian("spq_init\n");
    if (rhelp64[nr-1]==p64) nr--;
    if (!nr) Schlendrian("spq_init\n");
    spq_p_roots[i]=(u64_t *)xmalloc((nr+1)*sizeof(u64_t));
    spq_p_roots[i][0]=(u64_t)nr;
    memcpy(spq_p_roots[i]+1,rhelp64,nr*sizeof(u64_t));
  }

#if 1
  gmp_printf("%u spq (%Zd+): ",n_spq_q-2,first_spq1);
  for (i=1; i+1<n_spq_q; i++) printf("%u ",spq_q[i]);
  printf("\n");
...
@<Try to factor special q@>=
{
  ...
  if (mpz_cmp_ui(aux1,(ulong)pmax)<0) Schlendrian("spq_init\n");
  nb=mpz_sizeinbase(aux1,2);
  if (nb<=32) {
    tmpf[0]=mpz_get_ull(aux1);
    nf=1;
  } else {
/* The following can be removed if necessary. In this case make sure
   that mpqs or something else can factor the number. */
    if (nb>128) Schlendrian("spq_init: special q > 128 bit\n");
    if (psp(aux1)) {
      if (nb>64) continue;
      nf=1;
      tmpf[0]=mpz_get_ull(aux1);
      ...
      if (nf>8) continue; /* Should not happen. */
      for (k=0; k<nf; k++) tmpf[k]=mpz_get_ull(mf[k]);
    }
    qsort(tmpf,(size_t)nf,sizeof(u64_t),u64_cmp012);
    for (k=1; k<nf; k++) if (tmpf[k]==tmpf[k-1]) break;
    if (k<nf) continue; /* not square free */
  }
  for (k=0; k<nf; k++) if ((tmpf[k]<spq_pmin) || (tmpf[k]>spq_pmax)) break;
  if (k<nf) continue;
  for (k=0; k<nf; k++) {
  ...
  if (k<nf) continue;
  if (ndiv+nf>spq_ndiv_max) continue;
  adjust_bufsize((void**)&spq_p,&p_alloc,(size_t)(n_spq_p+nf),
                 16,sizeof(u64_t));
  for (k=0; k<nf; k++) { spq_p[n_spq_p]=tmpf[k]; n_spq_p++; }
}

@
@c
void spq_next(mpz_t spq)
{
  u32_t i;

  spq_q_ind++;
  if (spq_q_ind>=n_spq_q) Schlendrian("spq_next index too big\n");
  mpz_add_ui(spq,first_spq1,(ulong)(spq_q[spq_q_ind]));
  if (spq_q_ind+1==n_spq_q) return;
  mpz_set(aux1,spq);
  for (i=0,n_curspq_p=0; i<n_spq_p; i++) {
    mpz_set_ull(aux3,spq_p[i]);
    mpz_fdiv_qr(aux2,aux3,aux1,aux3);
    if (mpz_sgn(aux3)==0) {
      curspq_p[n_curspq_p]=spq_p[i];
      curspq_p_ind[n_curspq_p]=i;
      n_curspq_p++;
      mpz_set(aux1,aux2);
      if (mpz_cmp_ui(aux1,1)==0) break;
    }
  }
  if (mpz_cmp_ui(aux1,1)) Schlendrian("spq_next q\n");
  for (i=0,n_curspq_p32=0; i<n_curspq_p; i++)
    if (!(curspq_p[i]>>32)) n_curspq_p32++;
}


@
@c
void spq_crt(mpz_t a0, mpz_t m0, u64_t a1, u64_t m1)
{
  mpz_set_ull(aux3,m1);
  mpz_fdiv_r(aux2,a0,aux3);
  mpz_set_ull(aux1,a1);
  mpz_sub(aux1,aux1,aux2);
  mpz_mod(aux1,aux1,aux3);

  mpz_fdiv_r(aux2,m0,aux3);
  if (!mpz_invert(aux2,aux2,aux3)) Schlendrian("spq_crt: inversion failed\n");
  mpz_mul(aux1,aux1,aux2);
  mpz_mod(aux1,aux1,aux3);
  mpz_addmul(a0,m0,aux1);
/*
  modulo32=m1;
  x=modinv32((u32_t)(mpz_fdiv_r_ui(aux1,m0,(ulong)m1)));
  x=modmul32(modsub32(a1,(u32_t)(mpz_fdiv_r_ui(aux1,a0,(ulong)m1))),x);
  mpz_addmul_ui(a0,m0,(ulong)x);
*/
}
...
@
@c
u32_t spq_roots(mpz_t **rptr, size_t *alloc_ptr)
{
  u32_t i, j, k, nr, n0;
  u64_t n, *r0;
  mpz_t *r;
  ...
  n=1ULL;
  for (i=0; i<n_curspq_p; i++) n*=spq_p_roots[curspq_p_ind[i]][0];
  if (((size_t)n)>=*alloc_ptr) {
    size_t al, aln;

    al=*alloc_ptr;
    aln=16+(size_t)n;
    if (al) *rptr=(mpz_t *)xrealloc(*rptr,aln*sizeof(mpz_t));
    else *rptr=(mpz_t *)xmalloc(aln*sizeof(mpz_t));
    for (; al<aln; al++) mpz_init((*rptr)[al]);
    *alloc_ptr=aln;
  }
  r=*rptr;

  n0=(u32_t)spq_p_roots[curspq_p_ind[0]][0];
  r0=spq_p_roots[curspq_p_ind[0]]+1;
  for (j=0; j<n0; j++) mpz_set_ull(r[j],r0[j]);
  mpz_set_ull(aux4,curspq_p[0]);

  for (i=1; i<n_curspq_p; i++) {
    nr=(u32_t)spq_p_roots[curspq_p_ind[i]][0];
    r0=spq_p_roots[curspq_p_ind[i]]+1;
    for (j=1; j<nr; j++)
      for (k=0; k<n0; k++)
        mpz_set(r[j*n0+k],r[k]);
    for (j=0; j<n0*nr; j++) {
      k=j/n0;
      spq_crt(r[j],aux4,r0[k],curspq_p[i]);
    }
    mpz_mul_ull(aux4,aux4,curspq_p[i]);
    n0*=nr;
  }
  if (mpz_cmp(aux4,special_q)) Schlendrian("spq_roots\n");
  if (n!=n0) Schlendrian("spq_roots\n");
  return n;
}

@
@<Global decl...@>=
u64_t spq_i[20],spq_j[20],spq_x[20];
...
for (ind=0; ind<n_curspq_p; ind++) {
  if(((i64_t)b0)%((i64_t)curspq_p[ind]) == 0 &&
      ((i64_t)b1)%((i64_t)curspq_p[ind]) == 0) {
    i64_t x;

    x=a0%((i64_t)curspq_p[ind]);
    if(x<0) x+=(i64_t)curspq_p[ind];
    spq_i[ind]=x;
    x=a1%((i64_t)curspq_p[ind]);
    if(x<0) x+=(i64_t)curspq_p[ind];
    spq_j[ind]=(u64_t)x;
  } else {
    i64_t x;

    x=b0%((i64_t)curspq_p[ind]);
    if(x<0) x+=(i64_t)curspq_p[ind];
    spq_i[ind]=x;
    x=b1%((i64_t)curspq_p[ind]);
    if(x<0) x+=(i64_t)curspq_p[ind];
    spq_j[ind]=(u64_t)x;
  }
  modulo64=curspq_p[ind];
  spq_x[ind]=modmul64(spq_i[ind],(u64_t)i_shift);
}
}
...
@<Getopt@>=
{
  ...
  keep_factorbase=0;
  basename=NULL;
  mpz_init_set_ui(first_spq,0);
  mpz_init_set_ui(sieve_count,1);
  force_aFBcalc=0;
  sysload_cmd=NULL;
  process_no=0;
  catch_signals=0;
...
#define NumRead16(x) if(sscanf(optarg,"%hu",&x)!=1) Usage()
  ...
  append_output=0;
  while((option=getopt(argc,argv,"C:FJ:L:M:N:P:Q:R:S:ab:c:d:f:i:km:n:o:q:rt:vz"))!=-1) {
    switch(option) {
    case 'C':
...
//        if(sscanf(optarg,"%llu",&spq_pmin)!=1) Usage();
    ...
    case 'b':
      ...
      break;
    case 'c':
      if(gmp_sscanf(optarg,"%Zd",&sieve_count)!=1)  Usage();
      break;
    case 'f':
      if(gmp_sscanf(optarg,"%Zd:%Zd:%Zd",&first_spq,&first_spq1,&first_root)!=3) {
	if(gmp_sscanf(optarg,"%Zd",&first_spq)==1) {
	  mpz_set(first_spq1,first_spq);
	  mpz_set_ui(first_root,0);
	} else Usage();
      } else append_output=1;
      break;
    case 'm':
      if (sscanf(optarg,"%u:%u",rescale_bound,rescale_bound+1)!=2) {
        rescale_bound[1]=0;
        if (sscanf(optarg,"%u",rescale_bound)!=1) Usage();
      }
      break;
    case 'R':
      ...
      break;
    case 'd':
      if (sscanf(optarg,"%u",&spq_ndiv_max)!=1) Usage();
      break;
    case 'i':
...
#endif
  mpz_add(last_spq,first_spq,sieve_count);
#define SPQ_MAX   (1ULL<<50)
...
@<Get floating point coefficients@>=
{
  ...
  x=sqrt(mpz_get_d(first_spq)*sigma)*n_I;
  y=x/sigma;
  for(j=0;j<2;j++) {
...
if(mpz_sgn(sieve_count) != 0) {
  ...
  if(ofile_name==NULL) {
    char *fqstr, *lqstr;
    fqstr=mpz_get_str(NULL,10,first_spq);
    lqstr=mpz_get_str(NULL,10,last_spq);
    if(zip_output==0) {
      asprintf(&ofile_name,"%s.lasieve-%u.%s-%s",basename,
               special_q_side,fqstr,lqstr);
    } else {
      asprintf(&ofile_name,
	       append_output==0 ?
	       "gzip --best --stdout > %s.lasieve-%u.%s-%s.gz" :
	       "gzip --best --stdout >> %s.lasieve-%u.%s-%s.gz",
               basename,special_q_side,fqstr,lqstr);
    }
    free(fqstr); free(lqstr);
  } else {
...
@<Remove spq from factor bases@>=
{
  u32_t side,x,p,i,j,k,l,prime;
  u64_t p64;
  size_t sFB_alloc[2]={0,0};

  for(side=0;side<2;side++) {
    sFBs[side]=0;
    if (side!=special_q_side) continue;
    for (i=0,j=0; i<FBsize[side]; i++) {
      prime=FB[side][i];
      p64=(u64_t)prime;
/* TODO verbessern; spq_p ist bereits sortiert! */
      if (bsearch(&p64,spq_p,(size_t)n_spq_p,sizeof(u64_t),u64_cmp012)) {
        adjust_bufsize((void**)&(sFB[side]),sFB_alloc+side,
                       (size_t)(sFBs[side]+1),16,sizeof(**sFB));
        sFB[side][sFBs[side]].p=prime;
        sFB[side][sFBs[side]].r=proots[side][i];
        sFBs[side]++;
        continue;
      }
      if (j<i) {
        FB[side][j]=FB[side][i];
        proots[side][j]=proots[side][i];
      }
      j++;
    }
    FBsize[side]=j;
...
@<Prepare the factor base logarithms@>=
{
  ...
  for(side=0;side<2;side++) {
    ...
    if (side!=special_q_side)
      ...
      sieve_multiplier[side]=(UCHAR_MAX-50)/
                             (log(poly_norm[side])-log(mpz_get_d(special_q)));
#endif
    ...
    sFB_logs[side]=xmalloc(sFBs[side]);
    sFB_logss[side]=xmalloc(sFBs[side]);
    for (i=0; i<sFBs[side]; i++) {
      double l;

      l=log(sFB[side][i].p);
      sFB_logs[side][i]=rint(l*sieve_multiplier[side]);
      sFB_logss[side][i]=rint(l*sieve_multiplier_small[side]);
    }
  }
}
...
@<Generate |fbi_logbounds|@>=
{
  ...
  logbook(0,"Side %u maxl %lf  ",side,FB_maxlog[side]);
  for (l=0; l<10; l++) logbook(0,"%d ",(int)FB_logss[side][l]);
  logbook(0,"sm: %lf  %lf, max: %lf\n",sieve_multiplier[side],
          sieve_multiplier_small[side],log(poly_norm[side])/log(2.));
  for (l=0; l<10; l++) logbook(0,"%d ",(int)FB[side][l]);
  logbook(0,"\n");
/*
  logbook(-1,"Side %u maxl %lf  %d,%d  %lf\n",side,FB_maxlog[side],
          (int)FB_logs[side][0],(int)FB_logs[side][fbi1[side]-1],sieve_multiplier[side]);
*/
}
...
@<Prepare the lattice sieve scheduling@>=
{
  ...
  for(s=0;s<2;s++) {
    ...
    if(sigma>=1) pvl_max[s]=poldeg[s]*log(mpz_get_d(last_spq)*sqrt(sigma));
    else pvl_max[s]=poldeg[s]*log(mpz_get_d(last_spq)/sqrt(sigma));
...
static unsigned char *(sFB_logs[2]),*(sFB_logss[2]);
static unsigned char sieve_log_translate[2][256];
...
static u32_t rescale_bound[2], rescale[2];
...
@<Do the sieving and td@>=
{
  ...
  if (rescale[s]) {
    ...
    r=(1<<rescale[s])-1;
    for (iii=0; iii<L1_SIZE; iii++) {
      sieve_interval[iii]+=r;
      sieve_interval[iii]>>=rescale[s];
    }
    for (iii=0; iii<j_per_strip; iii++) {
      horizontal_sievesums[iii]+=r;
      horizontal_sievesums[iii]>>=rescale[s];
    }
#else
    if (rescale[s]==1) {
...
@<Prepare the auxilliary sieving data@>=
{
  u64_t absa0,absa1,absb0,absb1;
  char a0s,a1s;
  clock_t new_clock;

#define GET_ABSSIG(abs,sig,arg) if(arg>0) { abs=(u64_t)arg; sig='+';} \
        else { abs=(u64_t)(-arg); sig='-'; }
...
@<Preparations at the Archimedean primes@>=
{
  ...
  for(i=0;i<2;i++) {
    ...
    tpol64(tpoly_f[i],poly_f[i],poldeg[i],a0,a1,b0,b1);
    large_primes_summand=sieve_report_multiplier[i]*FB_maxlog[i];
    if(i==special_q_side)
      large_primes_summand+=sieve_multiplier[i]*special_q_log;
...
void do_scheduling(struct schedule_struct *sched,u32_t ns,u32_t ot,u32_t s)
{
  ...
  for(ll=0,ri=sched->ri;ll<sched->n_pieces;ll++) {
    ...
    if(ot==1)
      lasieve_setup64(FB[s]+fbi_lb,proots[s]+fbi_lb,fbi_ub-fbi_lb,
...
@ This is for primes which are divisors of some special q but not the current
special q.
@<Global decl...@>=
static u32_t *(smallsieve_auxs[2]), *(smallsieve_auxs_ub[2]);
static u32_t *(smallsieve_auxs_exc[2]), *(smallsieve_auxs_exc_ub[2]);
...
#define MAX_TINY_2POW 4
  ...
  MMX_TdAllocate(j_per_strip,fbis[0],fbis[1]);
    ...
    smallsieve_auxs[s]=xmalloc(4*sFBs[s]*sizeof(*(smallsieve_auxs[s])));
    smallsieve_auxs_exc[s]=xmalloc(sFBs[s]*sizeof(*(smallsieve_auxs_exc[s])));
  }
}
...
@<Preparation job for the small FB primes@>=
{
  ...
  for(s=0;s<2;s++) {
    ...
    for(fbi=0;fbi<fbis[s];fbi++) {
      ...
      aa=(u32_t)(absa0%((u64_t)FB[s][fbi]));
      if(a0s=='-' && aa != 0) aa=FB[s][fbi]-aa;
      bb=(u32_t)(absb0%((u64_t)FB[s][fbi]));
      if(proots[s][fbi]!=FB[s][fbi]) {
	u32_t x;
	x=modsub32(aa,modmul32(proots[s][fbi],bb));
	if(x!=0) {
	  aa=(u32_t)(absa1%((u64_t)FB[s][fbi]));
	  if(a1s=='-' && aa != 0) aa=FB[s][fbi]-aa;
	  bb=(u32_t)(absb1%((u64_t)FB[s][fbi]));
...
@<Preparation job for the small FB primes@>=
{
  ...
  for(s=0;s<2;s++) {
    u32_t i;
    u32_t *abuf; /* Affine */
    u32_t *ebuf; /* Exceptions */

    abuf=smallsieve_auxs[s];
    ebuf=smallsieve_auxs_exc[s];
    for(i=0;i<sFBs[s];i++) {
      u32_t aa,bb,p,r;

      p=sFB[s][i].p; r=sFB[s][i].r;
      if (mpz_fdiv_r_ui(aux3,special_q,(unsigned long int)p)==0) continue;
      modulo32=p;
      aa=(u32_t)(absa0%((u64_t)p));
      if(a0s=='-' && aa != 0) aa=p-aa;
      bb=(u32_t)(absb0%((u64_t)p));
      if(r!=p) {
	u32_t x;
	x=modsub32(aa,modmul32(r,bb));
	if(x!=0) {
	  aa=(u32_t)(absa1%((u64_t)p));
	  if(a1s=='-' && aa != 0) aa=p-aa;
	  bb=(u32_t)(absb1%((u64_t)p));
	  x=modmul32(asm_modinv32(x),modsub32(modmul32(r,bb),aa));
	  abuf[0]=p;
	  abuf[1]=x;
	  abuf[2]=(u32_t)(sFB_logss[s][i]);
	  abuf+=4;
	} else {
          *ebuf++=p; /* else infinity: TODO */
        }
      } else {
	/* Root is projective in (ab) coordinates. */
	if(bb!=0) {
	  ...
	  bb=(u32_t)(absb1%((u64_t)p));
	  abuf[0]=p;
	  abuf[1]=modmul32(asm_modinv32(x),bb);
	  abuf[2]=(u32_t)(sFB_logss[s][i]);
	  abuf+=4;
        } else {
          *ebuf++=p; /* else infinity: TODO */
    ...
    smallsieve_auxs_ub[s]=abuf;
    smallsieve_auxs_exc_ub[s]=ebuf;
  }
}
...
@<Preparation job for the small FB primes@>=
{
  ...
  for(s=0;s<2;s++) {
    ...
    for(x=smallsieve_aux[s];x<smallsieve_auxbound[s][0];) {
      ...
      } else x+=4; /* changed, since bound might be equal due to removal of spq divisors */
    }
...
@*3 Divisors of special q, ok at least for p<2^31-n_i.
@<Small sieve preparation for oddness type 1@>=
for(xx=smallsieve_auxs[s];xx<smallsieve_auxs_ub[s];xx+=4) {
  u32_t p;
...
for(xx=smallsieve_auxs[s];xx<smallsieve_auxs_ub[s];xx+=4) {
  ...
  p=xx[0];
  pr=xx[1];
  xx[3]= ( pr%2 == 0 ? ((i_shift+pr)/2)%p : ((i_shift+pr+p)/2)%p );
}
...
for(xx=smallsieve_auxs[s];xx<smallsieve_auxs_ub[s];xx+=4) {
  ...
  p=xx[0];
  pr=xx[1];
  xx[3]= ( pr%2 == 1 ? ((i_shift+pr)/2)%p : ((i_shift+pr+p)/2)%p );
}

@*3 Roots belonging to odd prime powers located precisely at infinty in the
projective space.
@<Small sieve preparation for oddness type 1@>=
...
@ Same thing for special q divisors.
@<Sieve with the small FB primes@>=
#if 1
{
  u32_t *x;

  for(x=smallsieve_auxs[s];x<smallsieve_auxs_ub[s];x+=4) {
    u32_t p,r,pr,d0;
    unsigned char l;
    ...
    if (p>=n_i) {
      for(d0=0;d0<j_per_strip;d0++) {
        if (r<n_i) {
          unsigned char *y;

          y=sieve_interval+(d0<<i_bits)+r;
          *(y)+=l;
        }
        r=r+pr;
        if(r>=p) r=r-p;
      ...
      for(d0=0;d0<j_per_strip;d0++) {
        ...
        y=sieve_interval+(d0<<i_bits); yy=y+r;
        while(yy<y+n_i) {
          *(yy)+=l;
          yy+=p;
        }
...
xFBtranslate(u16_t *rop,xFBptr op)
{
  ...
  p64=(u64_t)modulo32;
  rop[3]=op->l;
  am=a1>0 ? (u32_t)(((u64_t)a1)%p64) : modulo32-(u32_t)(((u64_t)(-a1))%p64);
  if(am==modulo32) am=0;
  bm=b1>0 ? (u32_t)(((u64_t)b1)%p64) : modulo32-(u32_t)(((u64_t)(-b1))%p64);
  if(bm==modulo32) bm=0;
  x=modsub32(modmul32(op->qq,am),modmul32(op->r,bm));
  am=a0>0 ? (u32_t)(((u64_t)a0)%p64) : modulo32-(u32_t)(((u64_t)(-a0))%p64);
  if(am==modulo32) am=0;
  bm=b0>0 ? (u32_t)(((u64_t)b0)%p64) : modulo32-(u32_t)(((u64_t)(-b0))%p64);
  if(bm==modulo32) bm=0;
...
for (ind=0; ind<n_curspq_p; ind++) {
  modulo64=curspq_p[ind];
  if (modadd64(modmul64(spq_i[ind],(u64_t)st_i),
               modmul64(spq_j[ind],(u64_t)true_j))==spq_x[ind]) break;
}
  if (ind<n_curspq_p) continue;
}
  cand[nc1++]=cand[ci];
}
rpol_eval_clear();
...
if(side==special_q_side) {
  u32_t ind;
  for (ind=0; ind<n_curspq_p32; ind++)
    *(fbp_ptr++)=curspq_p[ind];
}
...
if(side==special_q_side) {
  ...
  for(x=smallsieve_auxs_exc[side]; x<smallsieve_auxs_exc_ub[side];x++) {
    p=x[0];
    if (mpz_fdiv_qr_ui(aux2,aux3,aux1,p)==0) {
      mpz_set(aux1,aux2);
      *(fbp_ptr++)=p;
      while (1) {
        if (mpz_fdiv_qr_ui(aux2,aux3,aux1,p)) break;
        mpz_set(aux1,aux2);
        *(fbp_ptr++)=p;
      }
    }
...
@ Special q divisors above 32 bit: they are removed once. If one of these
divisors divides the value of the polynomial more than once, it has to be
found by the cofactorisation functions. 
@<Special q 64 bit td@>=
if(side==special_q_side) {
  u32_t ind;
  for (ind=n_curspq_p32; ind<n_curspq_p; ind++) {
    mpz_set_ull(aux3,curspq_p[ind]);
//gmp_printf("%Zd %Zd  %llu\n",aux1,aux3,curspq_p[ind]);
    mpz_fdiv_qr(aux2,aux3,aux1,aux3);
    if (mpz_sgn(aux3))
...
if(side==special_q_side) {
  ...
  for(x=smallsieve_auxs[side]; x<smallsieve_auxs_ub[side];x+=4) {
    u32_t p, pr, r;

    p=x[0]; modulo32=p;
    pr=x[1]%p;
    r=x[3]%p;
    if ((r%p)==((strip_i+modmul32((j_per_strip-strip_j)%p,pr%p))%p))
      *(fbp_ptr++)=p;
  }
}
