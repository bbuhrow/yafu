...
@
@c
void mpz_set_u64(mpz_t x, uint64_t src)
{
  #if GMP_LIMB_BITS==64
  mpz_set_ui(x, 1);	// to make sure it's allocated at the correct size
  x->_mp_d[0] = src;
  #elif GMP_LIMB_BITS==32
	#error "GMP_LIMB_BITS==32 not handled yet"
  #else
	#error "GMP_LIMB_BITS not a multiple of 32 is not handled yet"
  #endif
}

uint64_t mpz_get_u64(mpz_t x)
{
  #if GMP_LIMB_BITS==64
  return x->_mp_d[0];
  #elif GMP_LIMB_BITS==32
...
static int temp_alloced = 0;
static mpz_t gmpt;

void mpz_add_u64(mpz_t y, mpz_t x, uint64_t src)
{
  #if GMP_LIMB_BITS==64
  
  if (temp_alloced==0) { mpz_init(gmpt); temp_alloced=1; }
  
  mpz_set_ui(gmpt, 1);	// to make sure it's allocated at the correct size
  gmpt->_mp_d[0] = src;
  mpz_add(y, x, gmpt);
...
void mpz_tdiv_q_u64(mpz_t y, mpz_t x, uint64_t src)
{
	#if GMP_LIMB_BITS==64
...
void mpz_mul_u64(mpz_t y, mpz_t x, uint64_t src)
{
	#if GMP_LIMB_BITS==64
