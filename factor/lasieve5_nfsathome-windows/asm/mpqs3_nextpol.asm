...
dnl for (i=0; i<mpqs3_nFB; i++) {
dnl   p=fb[2*i];
dnl   mmi=mpqs3_FB_mm_inv[i];
dnl   pi=fbs[2*i]; bbb=fbs[2*i+1];
dnl   cc=bbb;
dnl   if (cc&1) cc+=p; cc>>=1;
dnl   cc=mpqs3_FB_disp[i]+(p-cc); if (cc>=p) cc-=p;
dnl   cc1=fb[2*i+1];
dnl   h32=cc1*pi;
...
# asm3_next_pol11_xmm(len)
function_head(asm3_next_pol11_xmm)
	movq %rdi,len    # len
	movq $0x00010001,%rax
	movq %rax,%r9
	shlq $32,%rax
...
dnl smjs	movq $mpqs3_FB_disp,disp_ptr
dnl smjs	movq $mpqs3_FB_np_px,p_ptr
dnl smjs	movq $mpqs3_FB_mm_inv,inv_ptr
dnl smjs	movq $mpqs3_FB_start,start_ptr
	leaq mpqs3_FB_disp(%rip),disp_ptr
	leaq mpqs3_FB_np_px(%rip),p_ptr
	leaq mpqs3_FB_mm_inv(%rip),inv_ptr
	leaq mpqs3_FB_start(%rip),start_ptr
...
# asm3_next_pol10_xmm(len,*invptr,*ropptr,bimul)
function_head(asm3_next_pol10_xmm)
	movq %rdx,ropptr
...
dnl asm3_next_pol3plus(len,*SI_add)
function_head(asm3_next_pol3plus)
dnl smjs	movq $mpqs3_FB_start,%rcx
dnl smjs	movq $mpqs3_FB_np_p,%rdx
	leaq mpqs3_FB_start(%rip),%rcx
	leaq mpqs3_FB_np_p(%rip),%rdx
...
dnl asm3_next_pol3minus(len,*SI_add)
...
dnl xmm functions
dnl
dnl asm3_next_pol3plus_xmm(len,*SI_add)
function_head(asm3_next_pol3plus_xmm)
dnl smjs	movq $mpqs3_FB_start,%rcx
dnl smjs	movq $mpqs3_FB_np_px,%rdx
	leaq mpqs3_FB_start(%rip),%rcx
	leaq mpqs3_FB_np_px(%rip),%rdx
...
np3plus_mainloop_xmm:
	...
	leaq 16(%rsi),%rsi
	leaq 32(%rcx),%rcx
	jnz np3plus_mainloop_xmm

	emms
	ret


dnl asm3_next_pol3minus_xmm(len,*SI_add)
function_head(asm3_next_pol3minus_xmm)
dnl smjs	movq $mpqs3_FB_start,%rcx
dnl smjs	movq $mpqs3_FB_np_px,%rdx
	leaq mpqs3_FB_start(%rip),%rcx
	leaq mpqs3_FB_np_px(%rip),%rdx
