...
#include <ht_intrinsics.h>	/* vendored trunk Montgomery set (montmul32_w /
				   montmul64_r convention), isolated from the
				   polysize cpu_intrinsics.h used by #2 */
...
typedef struct {
	...
	task_data_t *task;	/* carried to the shared handle_collision */
#ifndef NEW_P_PACKED
	p_packed_t *curr;
#endif
...
/*------------------------------------------------------------------------*/
/* trunk carries a uint64 special-q root; the shared handle_collision
   takes a uint128. Zero-extend. */
static uint128
ht_promote128(uint64 r)
{
	uint128 u;
	u.w[0] = (uint32)r;
	u.w[1] = (uint32)(r >> 32);
	...
	double *elapsed)
{
	...
	poly_search_t *poly = task->d->poly;
...
/*------------------------------------------------------------------------*/
/* contract entry points (see stage1_engine.h). This engine keeps no
   persistent per-thread hardware state -- sieve_specialq_64 allocates and
   frees its hashtable / invtable / progression arrays per call, which is
   acceptable overhead for a legacy baseline. */

void
cpu_hashtable_thread_data_init(void *data, int threadid)
{
	stage1_sieve_data_t *d = (stage1_sieve_data_t *)data;
	d->threads[threadid].hw_thread_data = NULL;
}

void
cpu_hashtable_thread_data_free(void *data, int threadid)
{
	(void)data;
	(void)threadid;
}
...
void
stage1_specialq_cpu_hashtable(task_data_t *task, uint32 threadid,
		...
		uint32 p_min, uint32 p_max)
{
	...
	int64 sieve_size = MIN((int64)(sieve_bound * pow((double)p_min, 4.)),
				(int64)SIEVE_MAX);
	double elapsed = 0;
	...
	sieve_specialq_64(task, sieve_size,
			...
			(uint32)special_q_min, (uint32)special_q_max,
			p_min, p_max,
			(double)task->coeff_deadline, &elapsed);

	t->cumulative_elapsed += elapsed;
}

#endif /* HAVE_CPU_HASHTABLE */
