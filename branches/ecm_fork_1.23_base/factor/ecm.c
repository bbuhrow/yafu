...
// declarations/definitions when using YAFU's ECM
	...
	void ecm_thread_init(z *n, int D, ecm_thread_data_t *tdata)
	{
		...
		//gmp_randseed_ui(tdata->params->rng, get_rand(&g_rand.low, &g_rand.hi));
                // use spRand to ensure that each thread gets a different random seed
		gmp_randseed_ui(tdata->params->rng, spRand(0, 0xffffffff));
		zCopy(n,&tdata->n);
...
int ecm_loop(z *n, int numcurves, fact_obj_t *fobj)
{
	...
	for (j=0; j < numcurves; j++)
	{
		...
		if (ECM_ABORT)
		{
                ...
                if (SIGMA != 0)
                {
                        if (*my_curves_run == 0 && total_numcurves > 1)
                                printf("WARNING: work will be duplicated with sigma fixed and numcurves > 1\n");
                        my_thread_data->sigma = SIGMA;
                }
                else if (get_uvar("sigma",&t))
                        my_thread_data->sigma = spRand(6,MAX_DIGIT);
                else
                {
                        if (*my_curves_run == 0 && total_numcurves > 1)
                                printf("WARNING: work will be duplicated with sigma fixed and numcurves > 1\n");
                        my_thread_data->sigma = (uint32)t.val[0];
                }
				...
				if (zCompare(&d,&zZero) == 0)
				{
					...
					if (isPrime(&return_factor[i]))
					{
						return_factor[i].type = PRP;
						add_to_factor_list(fobj, &return_factor[i]);
						...
						logprint(flog,"prp%d = %s (curve %d stg%d B1=%u sigma=%u thread=%d)\n",
							ndigits(&return_factor[i]),
							z2decstr(&return_factor[i],&gstr1),
							curves_run[i]+1,thread_data[i].stagefound,
							ECM_STG1_MAX,thread_data[i].sigma,i);
					}
					else
					{
						return_factor[i].type = COMPOSITE;
						add_to_factor_list(fobj, &return_factor[i]);
						logprint(flog,"prp%d = %s (curve %d stg%d B1=%u sigma=%u thread=%d)\n",
							ndigits(&return_factor[i]),
							z2decstr(&return_factor[i],&gstr1),
					...
					//reduce input
					zDiv(n,&return_factor[i],&t,&d);
					zCopy(&t,n);
					zCopy(n,&my_thread_data->n);
