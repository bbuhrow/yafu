...
#define MAX_LARGE_PRIMES 4
	...
	typedef struct {
		int64_t a;                // for siqs, index of a-poly in master list (was an int64)
		...
		uint32_t factor_list_word;  /* offset in an array of factors where
									all of this relations' factors appear, in order */
		uint8_t success;
	...
	typedef struct {
		...
		int lpba;
		int lpbr;
		int do_pm1;					// do P-1 pass before ECM
	...
	void relation_batch_free(relation_batch_t* rb, int has_prime_product);
