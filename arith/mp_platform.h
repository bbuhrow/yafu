...
/* ======================================================================
  ...
  #define _reset_lsb64(x) ((x) &= ((x) - 1))
    ...
    MP_FORCE_INLINE uint64_t _lead_zcnt64(uint64_t x) {
        uint32_t pos; return _BitScanReverse64(&pos, x) ? pos : 64;
    }
  #endif
  ...
  #define _trail_zcnt64 __builtin_ctzll
    ...
    MP_FORCE_INLINE uint32_t _lead_zcnt64(uint64_t x) {
        unsigned long pos; return _BitScanReverse64(&pos, x) ? (uint32_t)pos : 64;
    }
  #else
    MP_FORCE_INLINE uint32_t _trail_zcnt(uint32_t x) {
        uint32_t pos; return _BitScanForward(&pos, x) ? pos : 32;
...
/* --- add-carry / sub-borrow -------------------------------------------
   ...
   exists.  Taking uint64_t* here (and bouncing through an unsigned long long
   temporary for the native call) also removes the LP64 incompatible-pointer
   warning at call sites, where uint64_t is `unsigned long` but the intrinsic
   wants `unsigned long long*`.  The temporary costs nothing at -O1 and up.
  ...
  MP_FORCE_INLINE mp_carry_t mp_addcarry_u64(mp_carry_t c_in, uint64_t a,
      ...
      mp_carry_t c = (mp_carry_t)_addcarry_u64((unsigned char)c_in,
                         (unsigned long long)a, (unsigned long long)b, &t);
      *out = (uint64_t)t;
      return c;
  }
  MP_FORCE_INLINE mp_carry_t mp_subborrow_u64(mp_carry_t b_in, uint64_t a,
                                              uint64_t b, uint64_t *out) {
      unsigned long long t;
      mp_carry_t br = (mp_carry_t)_subborrow_u64((unsigned char)b_in,
                          (unsigned long long)a, (unsigned long long)b, &t);
      *out = (uint64_t)t;
      return br;
  }
  MP_FORCE_INLINE mp_carry_t mp_subborrow_u32(mp_carry_t b_in, uint32_t a,
      ...
      unsigned int t;
      mp_carry_t br = (mp_carry_t)_subborrow_u32((unsigned char)b_in, a, b, &t);
      *out = (uint32_t)t;
      return br;
  }
