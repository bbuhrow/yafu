...
void finalize_batchline(yafu_obj_t* yobj)
{
	if (yobj->USEBATCHFILE == 1)
	{
        ...
        {
            printf("warning: failed to remove _bkup temporary file with code %d\n", code);
        }
        if (code = remove("__tmpbatchfile"))
        {
            printf("warning: failed to remove __tmpbatchfile temporary file with code %d\n", code);
        }
	}
